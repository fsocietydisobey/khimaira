...
const MermaidBlock = lazy(() =>
  import("@/components/notebook/MermaidBlock").then((m) => ({
...
export function MarkdownView({
  ...
  // state across a content change. Safe even though `components` below is
  // memoized (not recreated every render): `seenSlugs` is a ref, so this
  // reset always runs at the top of THIS render, and `headingId` reads
  // `.current` at CALL time (when react-markdown invokes h1..h4 during
  // THIS render's reconciliation) — never a stale captured value.
  const seenSlugs = useRef<Map<string, number>>(new Map());
  seenSlugs.current = new Map();

  // Stable across renders (as long as `slugHeadings` doesn't change, which
  // it never does for a given surface) — this is what lets `components`
  // below be memoized instead of a fresh object every render. A fresh
  // `components` object every render was a real flicker bug (2026-07-04
  // report): react-markdown/React treat a changed `components.code` (etc.)
  // function reference as a different element type at that tree position,
  // forcing an unmount+remount of every heading/code-block/MermaidBlock on
  // every render — independent of whether `content` actually changed.
  const headingId = useCallback(
    (children: ReactNode): string | undefined =>
      slugHeadings
        ? slugifyHeading(headingText(children), seenSlugs.current)
        : undefined,
    [slugHeadings],
  );

  // The rest of these handlers (p, ul, ol, li, a, strong, blockquote, hr,
  // table, thead, th, td, pre, code) are pure — no props/state closure at
  // all — so `headingId` (used only by h1..h4) is this object's only real
  // dependency.
  const components = useMemo(
    () =>
      ({
        ...
        ul: (props) => (
          <ul
            className="mb-2 list-disc space-y-0.5 pl-4 text-muted-foreground"
            {...props}
          />
        ),
        ol: (props) => (
          <ol
            className="mb-2 list-decimal space-y-0.5 pl-4 text-muted-foreground"
            {...props}
          />
        ),

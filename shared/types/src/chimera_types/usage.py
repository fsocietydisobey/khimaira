...
Persisted as JSONL at `~/.local/state/chimera/usage.jsonl`. Read by:
...
ties back to the chimera dispatch that triggered it.
"""
...
class UsageRecord(BaseModel):
    ...
    task_id: str | None = Field(
        default=None,
        description=(
            "Chimera dispatch ID that triggered this call. None for ad-hoc "
            "calls outside of a chimera task. Per-task budget enforcement "

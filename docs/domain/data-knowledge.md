# Data Domain Knowledge — khimaira-dev
...
_Last meaningful update: 2026-05-26 by phase1-seed (initial template seed)_
...
_2026-05-26 by data-lead-1:_ The read → modify → atomic-rename pattern is
the standard for all JSONL mutation in sessions.py / chats.py — see
packages/khimaira/CLAUDE.md "Every primitive that mutates JSONL needs
round-trip coverage" for rationale and test pattern.
-->
...
<!-- Example entry:
_2026-05-22 by data-lead-1:_ consume_handoffs only rewrote handoffs.jsonl
when an unread+matching entry got its read_by updated → all-expired files
never triggered rewrite → file accumulated forever → caught by
test_consume_handoffs_expired_dropped (fix: always rewrite if any entry expired).
-->

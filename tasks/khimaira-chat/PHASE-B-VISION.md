...
- **Phantom-truncation across hops**: today's UserPromptSubmit inbox-surface auto-injector caps note bodies (~2500 chars per the spec, but observed ~600 in practice for some surfaces). When an agent receives a truncated body and re-relays it via chat_send, the truncation propagates silently — the receiver sees the cut version with no signal to refetch. Fixes to consider: (1) raise the surface cap, (2) emit a "fetch full body via session_pending_notes" hint in the truncation marker so receivers know to refetch, (3) sanitize-and-pass full body without preview clipping. Joseph hit this on the first relayed answer (cut at "Currently wait…" with ~620 chars displayed of an ~870-char source).

...
|---|---|---|
| **A — watchdog discovery drops prefixed-roster workers** | **BROKEN [audit-grade]** — CONFIRMED on live muther roster 2026-06-18 | My first-cut inspection-grade call (Path-B masquerade) was WRONG; the live probe REVERSED it (same shape as Specter Case Study 2). Root: `infer_role_from_name("muther-critic-1")` → **None** — it only strips a trailing `-<n>` then checks `_VALID_ROLES`, so `muther-critic` isn't a role. Discovery's `if not role: continue` (`roster_recovery.py:290`) then drops EVERY `muther-*`/`jp-*` worker window → `_discover_roster_windows` returned **0** for the live muther roster though all 10 seats were in-scope (`active_roster_member_ids`). `_process_window` never ran → **the watchdog has injected ZERO wakes, all-time, for any prefixed roster.** Obligation detection was never the problem (it works: `muther-tracker-1` had a live obligation + `unconsumed_chat=True` at probe time) — discovery drops the window BEFORE obligations are checked. Manual kitty nudges worked because they title-match directly, bypassing role resolution. **FIX (shipped):** prefix-tolerant resolver in discovery `_resolve_role` — try `infer_role_from_name(nm)`, then strip leading `<prefix>-` segments and retry (`muther-critic-1` → `critic-1` → `critic`). `roster_ids` filter still scopes to this daemon's roster (#19 intact). member_roles stores BASE roles, so the resolved base role aligns. Live-verified: discovery 0 → 8 muther windows. Tests: test_roster_recovery.py (strengthened test_normalizes_prefixed_names + new test_resolves_arbitrary_prefix_worker_roles). |

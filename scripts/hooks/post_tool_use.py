#!/usr/bin/env python3
"""chimera PostToolUse hook — auto-log file touches to the session store.

Runs after Edit / Write / MultiEdit / NotebookEdit. Reads the hook JSON
from stdin, extracts file path + tool name, appends to the chimera
...
Hard rules:
  ...
  - Stdlib only. The chimera package may not be importable from here.
...
def main() -> int:
    ...
    if not session_id or tool_name not in ("Edit", "Write", "MultiEdit", "NotebookEdit"):
        return 0

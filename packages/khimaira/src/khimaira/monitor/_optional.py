"""Lazy-import helper for optional `khimaira[monitor]` extras.
...
_INSTALL_HINT = "Install monitor extras: uv pip install 'khimaira[monitor]'"
...
def require(module_name: str) -> ModuleType:
    ...
    try:
        ...
        print(
            f"khimaira monitor: missing dependency `{module_name}` ({exc}).\n{_INSTALL_HINT}",
            file=sys.stderr,
        )
        sys.exit(1)

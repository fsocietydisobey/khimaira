...
khimaira's chain_pipeline (research → architect → implement → review)
...
Critical design choice — NEVER auto-merge. The flow is:
    ...
    chain_pipeline proposes a change-set
       ↓
    saved to ~/.local/state/khimaira/auto-fix/<timestamp>.md
       ↓
...
async def propose_fix(anomaly: dict[str, Any]) -> FixProposal:
    ...
    Architecture:
      1. Format the anomaly + recent context as a khimaira-pipeline task
    ...
    # even when khimaira-orchestration deps aren't installed.
    from khimaira.config.loader import load_config
...
def _format_pipeline_task(anomaly: dict[str, Any]) -> str:
    ...
    return (
        f"khimaira-monitor self-watch detected an anomaly. Investigate "
        ...
        f"## What to do\n"
        f"1. Identify the root cause (likely a bug in khimaira-monitor "
        ...
        f"## Constraints\n"
        f"- Modify ONLY khimaira's own source (under src/khimaira/).\n"
...
# (\\\\`{investigation_md.name}\\\\`) before running this — khimaira does NOT

...
Design notes:
  - Aggregation is per-NODE-NAME (not per-graph-and-node). Tradeoff:
    ...
    per project (verified for jeevy + khimaira), so this is fine in
  ...
  - Scope: only inspects threads where the latest activity is recent
    enough to matter. Configurable via KHIMAIRA_MONITOR_OBSERVATION_LIMIT
...
_DEFAULT_LIMIT = int(os.environ.get("KHIMAIRA_MONITOR_OBSERVATION_LIMIT", "200"))

...
When import fails (the project's deps aren't in khimaira's venv) or the
factory uses dynamic node names (khimaira's own factories!), the caller
...
def introspect_module(module_path: str, project_path: Path | None = None) -> list[TopologyResult]:
    ...
    Args:
        module_path: dotted module path like "khimaira.graphs.pipeline".

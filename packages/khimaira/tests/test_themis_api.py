...
def test_check_engine_runtime_error_fails_open(isolated_chats, sessions_mod, themis_client):
    """If themis.engine.evaluate() raises any non-import exception (bad YAML, etc.),
    check endpoint fails open (D7) — a broken rule file must not brick the hook."""
    s1 = "11111111-0000-0000-0000-000000000001"
    s2 = "22222222-0000-0000-0000-000000000002"
    _make_session(sessions_mod, s1)
    _make_session(sessions_mod, s2)
    ...
    assert r.status_code == 200
    assert r.json()["ok"] is True  # fail-open despite engine crash

...
from chimera.monitor.cli import (
    _LAUNCHD_LABEL,
...
def test_launchd_label_matches_convention():
    """launchd labels are reverse-DNS; `com.chimera.monitor` is what
    /api callers (doctor.py, attach.py) check via `launchctl list`."""
    assert _LAUNCHD_LABEL == "com.chimera.monitor"
...
def test_plist_content_routes_logs_to_library_logs():
    ...
    assert parsed["StandardOutPath"].endswith("chimera-monitor.out.log")
    assert parsed["StandardErrorPath"].endswith("chimera-monitor.err.log")
...
def test_install_launchd_on_darwin_without_launchctl_returns_1():
    ...
    args = argparse.Namespace(enable=False, force=False)
    with (
        patch("chimera.monitor.cli.sys.platform", "darwin"),
        patch("chimera.monitor.cli.shutil_which", return_value=None),
    ):
        rc = _cmd_install_launchd(args)
    assert rc == 1

"""`chimera usage savings` — savings math + mode attribution.
...
from chimera.cli import usage as usage_cli
...
def isolated_usage_log(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    from chimera import usage as usage_mod
...
def test_manual_and_explicit_tier_records_not_counted_as_savings(
    ...
    were the user's deliberate choice, not chimera's pick."""
    now = datetime.now(timezone.utc)

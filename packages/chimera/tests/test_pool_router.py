...
from chimera.dispatch.pool_router import (
    PoolDecision,
    derive_required_caps,
    select_from_pool,
)
from chimera.dispatch.registry import ModelCost, ModelEntry
...
def test_empty_pool_refuses_with_clear_message():
    ...
    assert "chimera models enable" in (d.refusal_reason or "")
...
def test_no_eligible_caps_refuses_with_hint():
    ...
    assert d.refused
    assert "chimera models" in (d.refusal_reason or "")
...
def test_audit_dict_includes_all_required_fields():
    """to_audit_dict must surface every field needed for `chimera usage

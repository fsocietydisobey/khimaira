...
Different from `chimera.dispatch.router.route()`: that one routes chain-task
...
Inputs:
  ...
  - The registry (defaults shipped + user override at ~/.chimera/models.yaml)
...
without them. With them, `chimera usage savings --audit` can show "of N
...
class PoolDecision:
    ...
    # Audit fields — for `chimera usage savings --audit` post-hoc review.
    ...
    if not eligible:
        ...
        if pool_size == 0:
            reason = (
                ...
                "enabled_for_auto=false. Run `chimera models enable <id>`."
            )
            ...
            reason = (
                ...
                f"enable a more-capable model in `chimera models`."
            )
        return PoolDecision(
            chosen=None,

...
A model registry is a list of every LLM the user wants chimera to
...
Load order:
  1. Shipped defaults (chimera/dispatch/default_models.yaml)
  2. ~/.chimera/models.yaml (user overrides — adds, removes, edits)
...
Used by:
  - chimera.dispatch.router for auto-mode model selection
  - mcp__chimera__delegate for tier resolution
  - chimera usage savings for cost calculations
  - chimera models {list,sync,add,enable,disable} CLI surface
"""
...
class ModelCost:
    ...
    change, users (or chimera releases) update the registry.
    """
...
def _user_registry_path() -> Path:
    """Per-user override path. Honors XDG_CONFIG_HOME, falls back to ~/.chimera/."""
    xdg = os.environ.get("XDG_CONFIG_HOME")
    if xdg:
        return Path(xdg) / "chimera" / "models.yaml"
    return Path(os.path.expanduser("~/.chimera/models.yaml"))
...
def _load_shipped() -> list[ModelEntry]:
    """Read the chimera-shipped default registry from package data."""
    try:
        pkg = resources.files("chimera.dispatch")
...
def load_registry() -> list[ModelEntry]:
    ...
    it's stable for `chimera models list` output.
    """
...
def auto_pool(registry: list[ModelEntry] | None = None) -> list[ModelEntry]:
    ...
    runtime check (see chimera.dispatch.runners.available_runners).
    """

...
A profile is a YAML file declaring everything `chimera bootstrap` and
`chimera sync` need to bring a fresh machine (or update an existing one)
to a known state:
...
profile applied on N machines yields N matching setups. Chimera ships
...
Resolution order when loading:
  ...
  2. CHIMERA_PROFILE env var
  3. ~/.config/chimera/profile.yaml
  4. Built-in default (chimera-only, no personal config)
"""
...
class DotfilesSpec:
    ...
    + symlink phase entirely (useful for "chimera-only, no personal
    config" setups).
    """
...
class SupervisorSpec:
    """Whether to install chimera's host-native supervisor (systemd
    on Linux, launchd on macOS). `auto_install=False` skips entirely —
    user can run `chimera monitor install-service` themselves later."""
...
class Profile:
    ...
    `mcp_servers` containing chimera itself — yields a working chimera
...
def load_profile(source: str | None = None) -> tuple[Profile, str]:
    ...
    Resolution order:
      ...
      2. CHIMERA_PROFILE env var
      3. ~/.config/chimera/profile.yaml
      4. Built-in default (chimera-only)
    ...
    env_src = os.environ.get("CHIMERA_PROFILE")
    if env_src:
        return _load_explicit(env_src), f"env CHIMERA_PROFILE={env_src}"

    xdg = os.environ.get("XDG_CONFIG_HOME") or os.path.expanduser("~/.config")
    user_path = Path(xdg) / "chimera" / "profile.yaml"
    if user_path.is_file():
        return _load_path(user_path), str(user_path)

    return _load_default(), "built-in default (chimera-only)"
...
def _load_url(url: str) -> Profile:
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "chimera-bootstrap"})
...
def dump_profile_json(profile: Profile) -> str:
    """Render a profile as JSON for diagnostic output (`chimera bootstrap

"""Per-operation idempotent helpers used by `chimera bootstrap` / `chimera sync`.
...
first successful run. This is what makes `chimera sync` cheap to run
...
def _claude_mcp_list() -> tuple[bool, set[str]]:
    ...
    MCP registration as skipped rather than failed (chimera bootstrap
...
def build_spa(chimera_root: Path) -> OpResult:
    """Build the chimera-monitor SPA bundle so the dashboard serves.
    ...
    Node and re-runs.
    """
    spa_dir = chimera_root / "apps" / "monitor-ui"
...
# ---------------------------------------------------------------------------
# Dotfiles sync (git pull) — used by `chimera sync`
# ---------------------------------------------------------------------------
...
def sync_dotfiles(spec: DotfilesSpec) -> OpResult:
    ...
    exists. Use in `chimera sync`, not first-run bootstrap.
    """
    path = Path(os.path.expanduser(spec.path)).resolve()
    if not (path / ".git").is_dir():
        return OpResult(
            ...
            target=spec.repo,
            status="failed",
            detail=f"no git repo at {path} — run `chimera bootstrap` first",
        )

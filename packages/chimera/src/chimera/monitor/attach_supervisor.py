"""Auto-reattach supervisor — runs inside the chimera-monitor daemon.

Two jobs:

1. **Startup pass** — on daemon boot, walk the registry of attached
   projects. For each one, verify chimera_observer is still in its venv;
...
Both jobs are idempotent. `chimera attach` may be called manually at any
...
from chimera.attach import attach_project, is_attached, list_attached, record_detach
from chimera.attach.inject import VenvNotFound, find_site_packages
    ...
    for entry in entries:
        ...
        if not project.is_dir() or not venv.is_dir():
            log.warning(
                ...
                "(remove via `chimera detach` if intentional)",
                project,
            )
            continue
        try:
...
async def watch_loop() -> None:
    ...
    try:
        ...
        log.warning(
            ...
            "`chimera attach` works manually. Install `watchfiles` to enable."
        )
        return

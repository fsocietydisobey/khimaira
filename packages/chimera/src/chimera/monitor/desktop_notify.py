"""Cross-platform desktop notifications for chimera cross-session events.
...
Disable globally with CHIMERA_DESKTOP_NOTIFY=0. Broadcast-on-decision
fan-out is OFF by default (too noisy); set CHIMERA_DESKTOP_NOTIFY_BROADCAST=1
to opt in.
...
def _enabled() -> bool:
    """Master switch. Default ON; set CHIMERA_DESKTOP_NOTIFY=0 to disable."""
    return os.environ.get("CHIMERA_DESKTOP_NOTIFY", "1") not in ("0", "false", "no")
...
def _broadcast_enabled() -> bool:
    ...
    """
    return os.environ.get("CHIMERA_DESKTOP_NOTIFY_BROADCAST", "0") in (
        "1",
        "true",
        "yes",
    )

...
The fire_swarm incident: a python process spawned a chimera swarm,
...
would need pg_stat_activity. Most chimera-driven runaways are SQLite-
...
long-running daemons (chimera-monitor itself, an active MCP server):
...
_PROCESS_AGE_HOURS = float(os.environ.get("CHIMERA_WATCHDOG_PROCESS_AGE_HOURS", "2.0"))
_DB_IDLE_HOURS = float(os.environ.get("CHIMERA_WATCHDOG_DB_IDLE_HOURS", "1.0"))

# A handful of cmdlines we KNOW are legitimate even when long-running.
# The chimera-monitor daemon itself shows up here whenever it has SQLite
# self-introspection happening; the chimera MCP server is the typical
...
_NEVER_ZOMBIE_PATTERNS = (
    ...
    "chimera_monitor",
    "uvicorn",
    "chimera/.venv/bin/chimera",  # the MCP server entry
)
...
def find_holders(
    ...
    *,
    ...
    for proc_dir in Path("/proc").iterdir():
        ...
        # Don't flag system processes — chimera-monitor, the MCP server,
...
def find_zombies(
    ...
    (chimera projects create per-graph DBs lazily) and return only

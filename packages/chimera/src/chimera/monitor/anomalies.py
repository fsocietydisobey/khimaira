...
async def check_usage_rate(
    ...
    *,
    ...
    Threshold lives in `CHIMERA_USAGE_ALARM_USD_PER_5M` (default $1.00).
    A token-rate alarm is also useful for unknown models that record
    $0; configurable via `CHIMERA_USAGE_ALARM_TOKENS_PER_5M`
    ...
    """
    threshold_usd = float(os.environ.get("CHIMERA_USAGE_ALARM_USD_PER_5M", "1.0"))
    threshold_tokens = int(os.environ.get("CHIMERA_USAGE_ALARM_TOKENS_PER_5M", "500000"))

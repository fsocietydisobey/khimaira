"""`chimera task` — context-resolved, auto-routed dispatch of a dev task.
...
from chimera.dispatch.router import route
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Register `chimera task` on the main argparse parser."""
...
async def _run_async(args: argparse.Namespace) -> int:
    from chimera.dispatch.runners.claude import ClaudeAuthError

    project = args.project or "."

    # Step 1 — classify
    print(f"[chimera task] classifying...", file=sys.stderr)
    t0 = time.monotonic()
    try:
        classification = await classify_task(args.description, project_path=project)
    except ClaudeAuthError as e:
        print(
            f"\n[chimera task] STOP — Claude rejected the call:\n  {e}\n"
            "Skipping retries to avoid burning more quota. "
            "Run `chimera doctor` to check available alternatives.",
            file=sys.stderr,
        )
        return 5
    except Exception as e:
        print(f"\n[chimera task] classifier failed: {e}", file=sys.stderr)
        return 1
    print(
        f"[chimera task] {classification.task_type}/{classification.complexity_tier} "
    ...
    if decision.refused:
        print(f"\n[chimera task] REFUSED: {decision.refusal_reason}", file=sys.stderr)
        return 2

    if decision.fallback_reason:
        print(f"[chimera task] note: {decision.fallback_reason}", file=sys.stderr)

    print(
        f"[chimera task] dispatch → {decision.chosen_runner}"
    ...
    if args.dry_run:
        print("\n[chimera task] dry-run — not dispatching", file=sys.stderr)
        return 0
    ...
    if not runner.is_available():
        print(
            f"\n[chimera task] runner {decision.chosen_runner} not available "
    ...
    try:
        ...
        print(f"\n[chimera task] runner {decision.chosen_runner} failed: {e}", file=sys.stderr)
        return 4

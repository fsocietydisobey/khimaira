"""`chimera mcp` — start the MCP server over stdio.

This is what terminal AI shells (Claude Code, Codex CLI, Gemini CLI) call.
Update `.claude.json` to point at:

    uv --directory /home/_3ntropy/dev/chimera run chimera mcp

The legacy invocation `uv run chimera` (no subcommand) defaulted to MCP;
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "mcp",
        help="Start the chimera MCP server over stdio (for AI CLI integration).",
        description=(
...
def run(_args: argparse.Namespace) -> int:
    ...
    # that we don't want loaded for `chimera doctor` or `chimera task`.
    from chimera.server.mcp import mcp

    # FastMCP's stdio runner takes over stdin/stdout. Logs go to stderr.
    print("[chimera mcp] starting MCP server over stdio", file=sys.stderr)
    mcp.run()
    return 0

"""`chimera tools` — discoverability surface for everything chimera exposes.

Lists CLI subcommands, MCP tools, slash commands, and web routes in one
catalog. Filterable by category or substring.

Usage:
    chimera tools                       # full catalog
    chimera tools session               # only items matching "session"
    chimera tools --category mcp        # only MCP tools
    chimera tools --json                # machine-readable
    chimera tools cost                  # everything related to cost
...
def _collect_web_routes() -> list[dict]:
    """Hardcoded list of chimera-monitor SPA routes worth surfacing.
...
_CATEGORY_LABELS = {
    ...
    "web": "🌐 chimera-monitor web routes",
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        ...
        description=(
            "List everything chimera exposes: CLI subcommands, MCP tools, "
            "Claude Code slash commands, chimera-monitor web routes, and "

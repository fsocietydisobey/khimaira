"""`chimera usage` — read the usage tracker, surface costs + savings.

Subcommands:
  chimera usage savings [--days N] [--by mode|runner|day]
  chimera usage list [--days N] [--mode auto|explicit-tier|manual]
...
Limitation (v1): savings here count only chimera-routed dispatches.
Direct Claude Code calls (the ones you make without going through
mcp__chimera__delegate / mcp__chimera__auto) aren't tracked yet —
that's the Phase 4 transcript scrape on the roadmap. Until then, this
number is the LOWER BOUND of what chimera saved you; real savings
are higher.
"""
...
from chimera.usage import estimate_cost, log_file_path
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Register `chimera usage` and its sub-subcommands."""
    parser = subparsers.add_parser(
        "usage",
        help="Inspect the usage tracker (cost, savings, breakdowns).",
        description=(
            "Reads ~/.local/state/chimera/usage.jsonl and reports cost + "
            "savings. The savings number compares what chimera actually "
    ...
    p_savings = sub.add_parser(
        "savings",
        help="Estimate USD saved by chimera auto-mode routing.",
    )
...
def _iter_records(days: int) -> Iterable[dict]:
    ...
    memory unless we have to (the file grows unbounded; `chimera usage
...
def _run_savings(args: argparse.Namespace) -> int:
    ...
    if not records:
        print(
            ...
            "Run a `mcp__chimera__auto` or `chimera task` dispatch to "

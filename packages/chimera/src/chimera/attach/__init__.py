...
`chimera attach <project>` drops chimera_observer + a .pth file into a
...
Production safety: the venv is gitignored, so chimera files never end up
in the committed source tree. Production builds use a fresh venv from
the project's manifest, which doesn't include chimera_observer.

The auto-reattach machinery (registry + watcher + daemon-startup re-pass)
lives in `chimera.monitor.attach_supervisor` to keep this module

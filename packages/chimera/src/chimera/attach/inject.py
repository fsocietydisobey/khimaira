"""Venv detection + chimera_observer file injection.

Pure file-system operations: find a project's venv, locate its
site-packages dir, copy `chimera_observer/` and `chimera_observer.pth`
...
# Directory containing the observer package source (bundled with chimera).
_OBSERVER_TEMPLATE = Path(__file__).resolve().parent / "observer_template"
_OBSERVER_PKG = _OBSERVER_TEMPLATE / "chimera_observer"
_OBSERVER_PTH = _OBSERVER_TEMPLATE / "chimera_observer.pth"
...
class AttachResult:
    ...
    package_written: bool      # True if chimera_observer/ was newly written or updated
...
def detect_venv(project_path: Path) -> Path:
    ...
    Honors a `CHIMERA_VENV` env override pointing at any directory.
    """
    import os

    override = os.environ.get("CHIMERA_VENV")
    if override:
        p = Path(override).expanduser().resolve()
        if _looks_like_venv(p):
            return p
        raise VenvNotFound(
            f"CHIMERA_VENV={override} doesn't look like a venv "
    ...
    raise VenvNotFound(
        ...
        "or set CHIMERA_VENV to point at the venv path."
    )
...
def is_attached(venv_path: Path) -> bool:
    """True iff chimera_observer.pth exists in the venv's site-packages."""
    try:
        sp = find_site_packages(venv_path)
    except VenvNotFound:
        return False
    return (sp / "chimera_observer.pth").is_file() and (sp / "chimera_observer").is_dir()


def attach_project(project_path: Path | str, *, force: bool = False) -> AttachResult:
    """Inject chimera_observer into a project's venv. Idempotent.
    ...
    pth_target = sp / "chimera_observer.pth"
    pkg_target = sp / "chimera_observer"
...
def detach_project(project_path: Path | str) -> AttachResult:
    """Remove chimera_observer from a project's venv. Idempotent."""
    project = Path(project_path).expanduser().resolve()
    venv = detect_venv(project)
    sp = find_site_packages(venv)
...
def _maybe_copy_package(target: Path, *, force: bool) -> bool:
    """Copy chimera_observer/ iff source has newer __init__.py (or force=True)."""

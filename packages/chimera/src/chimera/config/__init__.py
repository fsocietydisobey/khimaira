...
Resolution order (later overrides earlier):
  ...
  2. ~/.config/chimera/routing_table.yaml — user-wide
  3. <project>/.chimera/routing_table.yaml — per-project
  4. Environment variable overrides (e.g. CHIMERA_LOCAL_ONLY)
"""
...
def load_routing_table(project_path: str | None = None) -> dict[str, Any]:
    ...
    if project_path:
        proj_path = Path(project_path) / ".chimera" / "routing_table.yaml"
        if proj_path.exists():

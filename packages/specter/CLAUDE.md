...
- **Don't run Specter integration tests against your daily Chrome.** Start a dedicated test Chrome via `bin/specter-test-chrome` and set `SPECTER_TEST_PORT=9223` before running `pytest`. The dedicated Chrome is isolated (`--user-data-dir=/tmp/specter-test-profile`) — no shared cookies, history, or tabs with your real browser. Running against port 9222 navigates and mutates Joseph's active tabs. The `SPECTER_TEST_PORT` fallback to 9222 is deprecated (hard-skip after 2026-05-30).
